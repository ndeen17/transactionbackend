...
import { getPinStatus, resetPin, setupPin } from "../services/pin.service.js";
...
export const resetPinHandler = asyncHandler(async (req: AuthedRequest, res: Response) => {
  const { pin, currentPassword } = req.body as { pin: string; currentPassword: string };
  await resetPin({ userId: new Types.ObjectId(req.userId), pin, currentPassword });
  res.json({ success: true, data: { hasPin: true } });
});
